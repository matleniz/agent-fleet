...
- **Lean worker MCP profile** — workers inherit whatever MCP servers the
  machine/session has; their schemas are a per-session token tax (docs/06,
  operational levers). A per-project MCP allowlist applied by
  `pack_worker_setup` (for the CLIs whose config supports it) would keep
  workers lean by default.
